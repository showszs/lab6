...
test('Перевірка форми входу', async ({ page }) => {
    ...
    await page.click('#loinButton');
    await expect(page.locator('#succesMessage')).toBeVisible();
  });
...
test('Валідація обов’язкових полів фрми', async ({ page }) => {
    await page.goto('http://localhost:3000');
    await page.click('#loginButton');
    const error = await page.evaluate(() => document.queySelector(':invaid'));
